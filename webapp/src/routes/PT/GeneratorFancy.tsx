...
type Ex = { id:number, nombre:string }
...
export default function GeneradorFancy(){
  ...
  const generar = async()=>{
    const { data: ex } = await supabase.from('exercise').select('id,nombre,musculo').limit(200)
    const base = (ex||[])
    const tmp:Plan[] = []
    for(let d=1; d<=4; d++){
      const slice = base.slice((d-1)*5, (d-1)*5+5)
      tmp.push({ dia:d, ejercicios: slice.map(e=>({ id:e.id, nombre:e.nombre, series:4, reps:10 })) })
    }
  ...
  const publicar = async()=>{
    ...
    for(const d of plan){
      const { data: day } = await supabase.from('program_day').insert({ program_id: p.id, dia_index: d.dia }).select('id').single()
      for(const e of d.ejercicios){
        await supabase.from('program_exercise').insert({ program_day_id: day.id, exercise_id: e.id, series:e.series, reps:e.reps, descanso_seg: 90 })
      }
    }
  ...
  return <div className="container">
    ...
    {plan.map(d=>(
      <div key={d.dia} className="card">
        ...
        <ul>
          {d.ejercicios.map((e,i)=><li key={i} style={{padding:'6px 0'}}>{e.nombre} <span className="badge">{e.series}x{e.reps}</span></li>)}
        </ul>
      </div>
    ))}
